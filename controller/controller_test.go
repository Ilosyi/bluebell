...
func resetControllerDeps(t *testing.T) {
	...
	origUpdateUserProfile := updateUserProfile
	origCreatePost := createPost
	origCreateDraft := createDraft
	origGetPostByID := getPostByID
	origGetPostList := getPostList
	origGetPostListNew := getPostListNew
	origGetMyPostList := getMyPostList
	origGetManagePost := getManagePost
	origUpdatePost := updatePost
	origUpdateDraft := updateDraft
	origPublishDraft := publishDraft
	origDeletePost := deletePost
	...
	t.Cleanup(func() {
		signUp = origSignUp
		login = origLogin
		getProfile = origGetProfile
		updateUserProfile = origUpdateUserProfile
		createPost = origCreatePost
		createDraft = origCreateDraft
		getPostByID = origGetPostByID
		getPostList = origGetPostList
		getPostListNew = origGetPostListNew
		getMyPostList = origGetMyPostList
		getManagePost = origGetManagePost
		updatePost = origUpdatePost
		updateDraft = origUpdateDraft
		publishDraft = origPublishDraft
		deletePost = origDeletePost
